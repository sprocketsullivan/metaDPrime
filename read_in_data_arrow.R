...
setwd('.//dfa')
files<-list.files(pattern='*confidence_phase2.csv')#[-part.excl]
...
my.data$conf1 <- abs(as.numeric(my.data$PDW.response,na.rm=T)-0.5)
my.data[,zConf:=scale(as.numeric(conf1,na.rm=T)),by=participant]
...
# add type of trial
my.data$trialType<-ifelse(my.data$condition=="incongruent_squleft"|my.data$condition=="incongruent_squright" ,-1,ifelse(my.data$condition=="both_gratleft"|my.data$condition=="both_gratright" ,0,1))
my.data$trialType2<-factor(my.data$trialType, labels=c("invalid","inc","valid"))
...
save(my.data,file='MondayPilotData.RData') # need to figure where to save it so that it is private and easy to access
...
# dev.new() # to plot it into a new window

### Confifence Density Plots (see if the confidence varies with correct answer)
ggplot(subset(my.data,trialType==1), aes(x=zConf, fill=factor(key_resp_direction.corr))) +
geom_density(alpha=0.2) + facet_wrap(~ participant)+ylim(0,2)
...
summary_corr_choices<-
  group_by(my.data,participant,trialType2)%>%
...
ggplot(aes(y=mean_corr,x=trialType2,fill=trialType2),data=summary_corr_choices)+geom_bar(stat='identity',position="dodge")+
...
ggplot(aes(y=mean_corr,x=trialType2,fill=trialType2),data=summary_corr_choices)+geom_bar(stat='identity',position="dodge")+
  ...
  geom_hline(yintercept=0,linetype="dashed")+
  scale_fill_manual(values=c("#F5A503","#56D9CD", "#3AA1BF"))+ # isnt this the same as colourscheme at the beginning
  guides(fill = guide_legend(keywidth = 1.2, keyheight = 1.2))+ facet_wrap(~ participant)
